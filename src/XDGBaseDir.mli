...
(** XDG basedir implementation
    ...
    The XDG basedir specification makes clear where to store configuration,
    cache and data files. This a way to maintain a clean [$HOME]. Locations can
    be customized through environment variables ([XDG_DATA_HOME],
    [XDG_CONFIG_HOME]). It also allows to store files in several places (i.e.
    system directories).
